...
Existuje $2^n$ možností, kterými předměty vložíme do batohu. Naivní řešení prohledání všech možností tak běží v čase $\mathcal{O} (2^n)$.
...
Mohou existovat plány $p \in P_C$, na kterých nějaký tah nelze provést, čili $T(p)$ je nedefinované.
...
Plán optimální v ceně je takový plán, že jakýkoliv pokus o snížení ceny nějakým tahem snižujícím cenu způsobí, že výsledný plán bude odbavovat méně incidentů.
...
\subsection{Rekurzivní prohledávání stromu optimálních tahů}\label{kap:rekurzivniProhledavaniStromu}
...
\begin{veta}[Korektnost a úplnost algoritmu]
...
\begin{veta}[Složitost algoritmu rekurzivního prohledávání optimálních tahů]\label{veta:slozitostRek}
...
Hlavní výhodou algoritmu je schopnost plány prohledávat strukturovaně.
Díky tomu můžeme uvažovat i různé jiné varianty algoritmu, kdy například nemusíme strom rekurze prohledávat deterministicky,
ale stochasticky tak, že v každé hladině rekurze budeme optimální tahy náhodně permutovat.
Nemusíme ani na každé hladině zkoušet všechny optimální tahy, ale například pouze jeden náhodný, nebo nějakou část.
Oproti postupnému prohledávání všech tahů na každé úrovni má tento přístup výhodu ve vyzkoušení více odlišných konfigurací
a nalezené plány budou s menší pravděpodobností sdílet podobně naalokované týmy, vozidla a směny, což povede k diversifikovanějšímu prohledání množiny povolených plánů.
Při aplikování metod budeme uvažovat tuto lehce pozměněnou verzi algoritmu (viz kapitola \ref{kap:aplikaceTabu}).
...
Prostor prohledává prostřednictvním sousedů $N(x)$, kde vždy vybere souseda s nejlepší hodnotou účelové funkce.~(\citet{HybridMeta}, str. 3).
...
Pokud by $k$ i $|N(p)$| byly \uv{malé}, tak jsme získali velmi rychlý způsob, jak upravit libovolný plán, abychom dostali jeho \uv{nejlepší verzi}. 
...
Algoritmus \ref{alg:siman} simulovaného žíhání si v krocích 2 až 4 inicializuje aktuálně prohledávaný plán $p_{\text{current}}$, nejlepší nalezený plán $p_{\text{best}}$ a akutální teplotu $t$.
