...
Množina $P$ obsahuje všechny plány.
Ne všechny plány jsou pro nás zajímavé. Zajímat nás budou pouze plány, které splňují nějaké \textit{omezující podmínky}.
...
V kapitole \ref{kap:optUloha1ucObecne} definujeme cenu plánu \ref{df:cenaPlanu}.
...
Stav $S_A$ je rozmístění záchranných vozidel v prostoru. Stav $S_Z$ je informace, kdy bude zácrhranný tým k dispozici a pokud tým odbavuje nějaký incident, tak v jaké fázi odbavování zrovna je. 
...
\begin{algorithm}[h]
  ...
  \Function{Simulation}{$p, I$}
    ...
    \EndFor
    \State $s^I$ \gets \mbox{GetTimeWorking($S_Z$)}
    \State \Return $s_I$
  \EndFunction
  \end{algorithmic}
...
V kroku 18 simulace spočítá celkovou dobu záchranných týmů v terénu $s_I$ pomocí funkce \textit{GetTimeWorking} podle definice \ref{df:efPlanu}, na základě $S_Z$.
...
Simulace vrátí $s_I$, tudiž $s_I = s(p, I)$.
