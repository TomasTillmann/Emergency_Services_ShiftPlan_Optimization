...
\textit{Pracovní směna} $d \in D$ je tak dvojice $d = (d_s, d_l)$, kde $d_{1} \in D_{1}$ značí začátek a $d_{2} \in D_{2}$ délku trvání směny.
Nechť $D_n = |D|$.
...
$p_{Z}(z) = v_{\emptyset}$ a $p_{D}(z) = (d_s, d_l) \colon d_l - d_s = 0$.
...
\begin{align*}
  ...
  & p_{d_s}(z) \colon  p_D(z) = (d_s, d_l) \Leftrightarrow p_{d_s}(z) = d_l.
\end{align*}
...
Ten určujeme podle počtu úspěšně odbavených incidentů $s_I$ a ceny plánu $u$.
...
$x^* \in \mathcal{X}$ takzvaný \textit{utopia point} $y^{utopia}$ (\citet{AlgOptBook}, str. 219).
...
V nadcházející kapitole \ref{kap:metodyProReseniOptSViceUcel} si ukážeme jak se optimalizace takového typu řeší.

V předchozí kapitole jsme si ukázali způsob, jakým nalezneme počet úspěšně odbavených incidentů $s_I$. 
Nyní si definujme cenu plánu. 
\begin{definice}[Cena plánu $u$]\label{df:cenaPlanu}
  Cena plánu $u \colon P \rightarrow \mathbb{N}$,
  \begin{align*}
    u(p) = \sum_{d_l = p_{d_s}(z), z \in Z} d_l + |\{ a \in A \mid p_{A}(a) \neq v_{\emptyset} \}|.
  \end{align*}
...
Ukažme si, jak problém nalezení optimálního plánu modelovat jako optimalizační úlohu s více účelovými funkcemi.
\begin{definice}[Optimalizační úloha nalezení optimálního plánu pohotovostní služby jako optimalizační úloha s více účelovými funkcemi]\label{df:optUloha2uc}
  ...
  , $u'$ a $s'$ jsou přeškálované účelové funkce \ref{df:simulaceSkal} \ref{df:cenaPlanuSkal} a $I$ je daná množina incidentů.
\end{definice}

Účelové funkce jsou přeškálování počtu úspěšně odbavených incidentů $s'(p, I)$ a přeškálování záporné ceny plánu pohotovostní služby $u'(p)$, $p \in P_C$.
Dají se ekvivalentně použít i původní funkce $s$ a $u$.
...
Z toho důvodu řešení $y^{utopia}$ nemusí existovat. Proto se v další sekci podíváme, jak úlohu nalezení optimálního plánu převedeme na optimalizační úlohu s jednou účelovou funkcí,
kde alespoň jedna optimální konfigurace vždy existuje.

\section{Metody pro řešení optimalizačního problému s více účelovými funkcemi}\label{kap:metodyProReseniOptSViceUcel}

V předchozí kapitole jsme problém nalezení optimálního plánu pohotovostní služby namodelovali jako optimalizační úlohu s více účelovými funkcemi \ref{df:optUloha2uc},
sice přeškálování počtu úspěšně odbavených incidentů $s'(p, I)$ a ceny plánu $u'(p)$.
...
Důvodem je, že řešení optimalizační úlohy s jednou účelovou funkcí je dobře definované,
narozdíl od řešení optimalizační úlohy s více účelovými funkcemi.

V této kapitole si vybrané metody ukážeme a rozmyslíme si, které budeme preferovat.
...
\begin{alignat*}{2}
  \min_{p^* \in P} \{ u(p^*, I) \}, && \hspace{15pt} P = \{ p \mid \max_{p \in P_C} \{ s(p, I) \} \},
\end{alignat*}
pro danou množinu incidentů $I$.
...
Proto jsme si v předchozí kapitole \ref{kap:metodyProReseniOptSViceUcel} ukázali několik přístupů,
...
V této kapitole nadefinujeme problém nalezení optimálního plánu jako optimalizační úlohu s jednou účelovou funkcí.
Označme si $Q_I = \{ q^{\text{Lex}}, q^{\text{goal}}, q_{\alpha}, q_{\alpha p}\}$ jako množinu účelových funkcí na dané množině incidentů $I$.
\begin{definice}[problém nalezení optimálního plánu jako optimalizační úloha s jednou účelovou funkcí]
  \begin{align}
    \max_{p \in P_C} \{ q(p) \},
  \end{align}
  kde $q \in Q_I$.
...
V předchozí kapitole \ref{kap:opt1Uc} se nám podařilo problém nalezení optimálního plánu namodelovat jako optimalizační úlohu s jednou účelovou funkcí,
...
\begin{dukaz}
  ...
  Z lemma (\citet{Diskretka}, str. 61) víme, že takových uspořádaných $V_n$-tic je
...
\begin{dukaz}
  Odhadněme s pomocí lemma (\citet{Diskretka}, str. 62) a lemma (\citet{Diskretka}, str. 63)
...
\begin{dukaz}
  Definiční obor přiřazujících funkcí $p_Z$, $p_A$, $p_{D_{1}}$, $p_{D_{2}}$, jsou množiny $Z$, $A$, $D_S$, $d_s$, obsahující konečný počet objektů.
...
Ukažme si první, do jaké třídy složitosti náleží optimalizační úloha, na jejíž účelovou funkci dáme jisté omezení \ref{df:Q}.
