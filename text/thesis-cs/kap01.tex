...
Budeme proto psát ekvivalentně $s'_c(p)$ a $u'(p)$ a předpokládat $I$ jako stejnou a pevně zvolenou, pokud nebude řečeno jinak. 
...
Jedná se tedy o optimalizaci po jednotlivých účelových funkcích podle určené priority. 
Není obecně vhodné, protože neumí nalézt kompromis mezi jednotlivými účelovými funkcemi.

\begin{definice}[Lexikografické porovnávání optimalizace pohotovostního plánu]\label{df:lexPorovnaniPohotovost}
  \begin{align*}
    q^{\text{Lex}}(p_1) < q^{\text{Lex}}(p_2) \Leftrightarrow ( s'_c(p_1) < s'_c(p_2)) \lor (s'_c(p_1) = s'_c(p_2) \land u'(p_1) < u'(p_2))
  \end{align*}
  a 
  \begin{align*}
    q^{\text{Lex}}(p_1) = q^{\text{Lex}}(p_2) \Leftrightarrow s'_c(p_1) = s'_c(p_2) \land u'(p_1) = u'(p_2).
  \end{align*}
\end{definice}
