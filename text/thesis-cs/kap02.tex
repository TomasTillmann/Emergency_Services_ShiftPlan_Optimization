...
\begin{algorithm}[H]
  \caption{Simulated Annealing Function}
  \begin{algorithmic}[1]
    ...
    \While{$t > t_{\text{final}}$}
      \For{$m \gets 1$ \textbf{to} $M_k$}
        \State $p \gets \text{zatím neprohledaný } \in \text{neighbours}(p_{\text{current}})$
...
Algoritmus si v krocích 2 až 4 inicializuje aktuálně prohledávaný plán $p_{\text{current}}$, nejlepší nalezený plán $p_{\text{best}}$ a akutální teplotu $t$.
...
\end{align*}
Díky tomu je umožněno akceptovat i lokálně horší řešení.
