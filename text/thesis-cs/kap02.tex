...
Triviálním příkladem využití dynamického programování je výpočet $n$-tého \textit{Fibbonaciho čísla}. (\cite{mares}, kap. 12).
...
\begin{dukaz}
  ...
  První \ref{en:_1} a druhý \ref{en:_2} případ pokrývají situaci, kdy $z$ odbavuje pouze $i_k$.
  Třetí \ref{en:_3} a čtvrtý \ref{en:_4} případ pokrývají situaci, kdy $z$ odbavuje i nějaké jiné incidenty a $z$ jde zkrátit směnu, aby stále odbavoval všechny incidenty $I_z$.
...
Pro zjištění všech optimálních plánu $P^*$ na množině incidentů $I$ algoritmus (viz algoritmus \ref{alg:rekProhPlanu}) zavoláme: $P^* = $ OptimalMovesSearch($p_{\emptyset}$, $I_1$, $|I|$).

Algoritmus prohledávání plánů optimálními tahy si v kroku 7 vygeneruje všechny optimální tahy, které lze provést na incidentech $I_k$ a plánu $p$.
Následně je v krocích 9 až 20 iteruje.

V kroku 10 se rekurzivně zavolá sám na sebe.
V argumentech předá optimální plán v ceně získaného z aktuálního plánu a z právě iterovaného optimálního tahu,
o jedna větší množinou incidentů a maximální hloubkou rekurze, která v každé hladině rekurze činí velikost všech incidentů $I$.

V krocích 11 a 12 vybere reprezentanta doposud nalezených optimálních plánů v ceně $P^*$ a nově nalezených optimálních plánů v ceně $P'^*$, vrácených z rekurze.
Můžeme vybrat libovolně, protože všechny $P^*$ a $P'^*$ mají mezi sebou stejná ohodnocení.
Pokud se jedná o první iteraci optimálních tahů, tak v kroku 14 nastavíme $P^*$ na $P'^*$.

V kroku 15 testujeme, jestli se ohodnocení rovnají. Pokud ano, tak je sjednotíme v kroku 16.
Pokud ovšem nově nalezené optimální plány v ceně mají lepší ohodnocení, testuje se v kroku 17, tak jimi $P^*$ nahradíme. 
Tím se zajistí, že po proiterování všech optimálních tahů jsou doposud nalezené optimální plány v ceně s nejnižší možnou cenou.

V kroku 2 rekurze pouze vrátí optimální plán v ceně s předchozí hladiny, pokud $k$ je rovno $h$, čili $p$ je nějaký optimální plán v ceně na množine $I$.

Algoritmus si \textit{cachuje} mezivýsledky, v kroku 21 uloží nejlepší optimální plány v ceně pro aktuální plán.
Pokud v rekurzi nějaký plán navštívíme vícekrát, což se jistě může stát, protože různé posloupnosti tahů můžou vést na stejný plán,
tak se nebude znovu prohledávat celý podstrom rekurze, ale jednoduše se v kroku 5 vrátí správné optimální plány v ceně, nalezené v předchozích voláních.

Jak již bylo zmíněno v kapitole \ref{kap:dynamicProgram}, tomuto způsobu \textit{cachování} se v dynamickém programování říká memoizace a je to velmi účinná technika zrychlení algoritmu.

Právě jsme si algoritmus důkladně popsali, v následující větě ověříme, že je korektní, tedy že skutečně nalezne všechny optimální plány na množine $I$ při $q^{\text{Lex}}$.

\begin{veta}[Korektnost a úplnost algoritmu]\label{}
  Algoritmus rekurzivního prohledávání prostoru plánů ptimálními tahy (viz algoritmus \ref{alg:rekProhPlanu}) nalezne všechny optimální plány $P^* \in P_C$ při účelové funkci $q^{\text{Lex}}$.
\end{veta}
\begin{dukaz}
  Z věty \ref{veta:dosazitelnostOptimalnimiTahy} víme, že $p^*$ je dosažitelný nějakou posloupností optimálních tahů z prázdného plánu $p_{\emptyset}$. 
  V algoritmu v kroku 7 množina $T^*_k$ obsahuje všechny optimální tahy, které lze provést na plánu $p$ na množině $I_k$.
  V kroku 10 se rekurzivně zavoláme na každý $T^*(p), T^* \in T^*_k$.
  ...
  V krocích 13--19 z plánů, které se vrátí rekurzí, vybíráme pouze ty optimální vůči $q^{\text{Lex}}$.
  Vybíráme tak ze všech plánů, které jsou dosažitelné nějakou posloupností optimálních tahů ty optimální při $q^{\text{Lex}}$.
  Zároveň i cache je korektně používaná, tudiž algoritmus je i korektní.
\end{dukaz}
...
\begin{dukaz}
  ...
  aby byly odbavovány všechny incidenty $I_{k+1}$.
  Takové prodloužení opět, tentokrát pro každý naalokovaný tým existuje právě jedno.
...
Sousedy $N(p)$ zkonstruujeme implicitně pomocí tahů, což je standardní způsob konstrukce sousedů nějaké konfigurace (\cite{HybridMeta}, str. 3).
...
\begin{enumerate}
  ...
  \item $\forall p \in P_C \colon p' \in N(p)$, jsou si plány $p$ a $p'$ podobné, čili z větší části mají naalokovaná stejná vozidla a týmy, se stejnými směnami. \label{en:t}
\end{enumerate}

První podmínka \ref{en:f} zakazuje navštěvování nedovolených plánů. Pro některé problémy je navštěvování nedovolených konfigurací žádoucí,
protože omezující podmínky můžou být příliš svazující (\cite{GlovKoch03} str. 50--51).
To ale není náš případ, čili se můžeme omezit pouze na dovolené plány.

Druhá podmínka  \ref{en:s} zajišťuje, že při prohledávání je vůbec možné globální optimum navštívit.

Třetí podmínka \ref{en:t} vyžaduje, aby sousední plány sdíleli s původním plánem nějaké komponenty. 
...
\begin{veta}[Časová složitost lokálního prohledávání]
  Nechť libovolný plán $p \in P_C$, pak lokální prohledávání běži v čase
  \begin{align*}
    \mathcal{O}(k \cdot |N(p)|),
  \end{align*}
...
Algoritmus \ref{alg:tabusearch} si v krocích 1--5 inicializuje stav algoritmu. Tabu je ze začátku prázdné.
...
Takové aspirační kritérium je nejčastěji používané a obecně nejefektivněšjí aspirační kritérium~(\cite{GlovKoch03} str. 47).
...
\end{align}
Pravděpodobností funkce \ref{df:metropolis} je takzvané \textit{metropolis akceptační kritérium} (\citet{Metropolis}), které modeluje
